...
```{r, echo=FALSE}
mergedfinal <- read.csv("county_index.csv")
...
write.csv(data_all, "county_collectivism_score_all.csv")
...
```{r, echo=FALSE}
county_score <- read.csv("county_collectivism_score.csv")
state_score <- read.csv("/Users/erin/Desktop/state_collectivism/state_collectivism_score.csv")

state_score <- state_score %>% 
  select(state, state_collectivism_score)

county_score <- county_score %>% 
  select(state, county, county_collectivism_score)
...
new_score <- state_score %>% 
  left_join(county_score_group, by = "state")

#check correlation between state level col score and average score: r=0.866
cor.test(new_score$avg_county_collectivism_score, new_score$state_collectivism_score)
...
write.csv(score_all, "score_all.csv")
